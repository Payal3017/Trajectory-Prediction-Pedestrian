...
def load_crowds(path, **kwargs):
        ...
        if data[i]:
            for step in range(1, len(data[i])):
                if data[pNumber][step][2] == frame_number:
                    ...
                    print('speed --> pixel per second :', speed)
            ...
            print('Distance from destination (pixel) :', distDes)
    ...
    for i in range(start, end):
        if data[i]:
            ...
            print(' Nearest Neighbour of person =' + str(p1+1) + ' is person = ' + str(p2+1) + 'at a distance',
                              NND)
